...
import { Calendar, Package, Clock, CheckCircle, AlertCircle, Phone, MapPin } from "lucide-react"
import { DUMMY_BOOKINGS } from "@/lib/data"

export function StaffDashboard() {
  const todayPickups = DUMMY_BOOKINGS.filter(
    (booking) =>
      booking.status === "confirmed" && new Date(booking.startDate).toDateString() === new Date().toDateString(),
  )

  const pendingReturns = DUMMY_BOOKINGS.filter((booking) => booking.status === "picked-up")

  const overdueReturns = DUMMY_BOOKINGS.filter((booking) => booking.status === "overdue")
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{pendingReturns.length}</div>
            <p className="text-xs text-muted-foreground">Items to be returned</p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Scheduled pickups for {new Date().toLocaleDateString()}</CardDescription>
          </div>
        ...
        <CardContent>
          {todayPickups.length > 0 ? (
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs">Pickup: {new Date(booking.startDate).toLocaleTimeString()}</span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="text-center py-8">
              ...
              <h3 className="font-semibold text-lg mb-2">No pickups scheduled</h3>
              <p className="text-muted-foreground">All scheduled pickups for today are completed</p>
            </div>
          )}
        </CardContent>
      </Card>
              ...
              {pendingReturns.map((booking) => {
                ...
                return (
                  <TableRow key={booking.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${getPriorityColor(isOverdue)}`} />
                        <span className="text-xs">{isOverdue ? "Overdue" : "Normal"}</span>
                      </div>
                    </TableCell>
